...
| --- | --- |
| **Intelligence › Sources** | The systems CORE is allowed to read, one card each: the payment system, the supplier file, the ledger. Each card says whether the source was reachable when last checked, or that it has not been checked. The credentials for each source are held apart from its settings and encrypted, and adding or changing a source can be limited to a named list of administrators. |
...
| **Intelligence › Resolve** | The estate grouped into business domains, with every link between datasets marked as declared by the source or inferred from the data. It also shows where what a source declares about itself disagrees with what its data shows. For a supplier question such as "which datasets could this provider's systems read?", this is where the answer starts. |
