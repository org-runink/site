...
import { waived } from './contrast-waivers.mjs';
...
for (const ground of ['sheet', 'console']) {
  ...
  /* EVERY SURFACE, not just the canvas. This loop used to measure each
     category accent against --rk-ground alone, which is the same mistake the
     ink tiers made before --rk-text-3 was caught at 4.39:1 on --rk-sunk: a
     page does not paint accent text only on the canvas. On the console ground
     three accents are below AA on --rk-sheet, and the table that is supposed
     to be the authority on the palette did not have a row for the pair.
     Waived failures are graded "FAILS (waived)" — they publish as failures on
     /design/, and --check does not go red on them. See
     scripts/contrast-waivers.mjs. */
  for (const cat of CATS) {
    ...
    for (const [sTok, sName] of SURFACES) {
      ...
      if (!b) continue;
      const r = ratio(f, b);
      const { covered } = waived(ground, cat, sTok, r);
      rows.push({ f, fn: `cat ${cat}`, b, bn: label(sName), r: r.toFixed(2),
                  g: r >= 7 ? 'AAA' : r >= 4.5 ? 'AA' : (covered ? 'FAILS (waived)' : 'FAILS'),
                  tier: 'ink' });
    }
  }
}
...
const waivedPairs = rows.filter((r) => r.g === 'FAILS (waived)');
...
if (CHECK) {
  ...
  if (waivedPairs.length) {
    console.warn(`gen-contrast-proof: ${waivedPairs.length} pair(s) below floor, REPORTED NOT GATED — see scripts/contrast-waivers.mjs`);
    for (const p of waivedPairs) console.warn(`  ~ ${p.fn} (${p.f}) on ${p.bn} (${p.b}) = ${p.r}:1`);
  }
  console.log(`gen-contrast-proof: ok — ${rows.length} pairs, ${waivedPairs.length} waived, the rest at or above their floor, table matches tokens.css`);
} else {
