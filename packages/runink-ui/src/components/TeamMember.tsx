...
export function TeamMember({ name, role, image, imageAlt, linkedinUrl, className }: TeamMemberProps) {
  ...
  return (
    <div className={cx('w-64 text-center', className)}>
      <div className="relative mb-4 aspect-square overflow-hidden rounded-card">
        {showImage ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-surface-raised">
            {initials ? (
              <span aria-hidden="true" className="text-4xl font-bold tracking-wide text-secondary">
                {initials}
              </span>
            ) : (
              <svg className="h-20 w-20 text-secondary" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        {profileUrl && (
          <a
            ...
            className="absolute bottom-2 right-2 rounded-full bg-surface p-2 shadow-md transition-shadow hover:shadow-lg"
          >
            <svg className="h-5 w-5 text-ink-accent" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <h3 className="mb-1 text-xl font-bold text-primary">{name}</h3>
