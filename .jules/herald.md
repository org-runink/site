...
## 2026-05-17 - Jest testing DOM attributes with Mock
**Learning:** Testing logic that depends on `dataset` vs `getAttribute` natively needs appropriate parsing or fallback assertions to verify correct assignments, rather than overriding assertions entirely.
**Action:** When fixing Jest tests with JSDOM, ensure that properties like `parallaxDepth` are initialized safely without introducing tautologies or bypassing core test behaviors. Make sure the container ID/classes align exactly with what the script targets (e.g., `id="hero-parallax"` vs `class="hero-parallax-container"`).
