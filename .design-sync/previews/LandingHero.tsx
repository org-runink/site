 ...
 *
 * Opacity only, no `mix-blend-*`. These planes used to carry `luminosity`,
 * `color-burn` and `screen`, all three tuned by eye against the dark ground, and a
 * grader on the sheet cell found the result: `screen` against near-white is a
 * mathematical no-op, so the TRUCKS plane vanished outright, and the other two
 * bleached to value-less ghosts leaving only stray artefacts. A blend mode is a
 * function of the backdrop, so no single one can serve two grounds — `screen`
 * lightens and `multiply` darkens, and the ground decides which is destructive.
 * `LandingHero` now composites with plain alpha and forces `mix-blend-mode: normal`
 * on every plane, so passing one here would be overridden anyway.
 */
const LAYERS: LandingHeroLayer[] = [
  { src: WAREHOUSE, alt: 'Warehouse facility background', depth: 0.1, className: 'opacity-70' },
  { src: DISTRIBUTION, alt: '', depth: 0.3, className: 'opacity-80' },
  { src: TRUCKS, alt: '', depth: 0.6, className: 'opacity-90' },
];
 ...
 * with their own palette baked into the SVG. What they no longer do is *invert*:
 * they used to carry blend modes tuned against a dark band, and on this ground
 * `screen` made the nearest plane disappear completely while the other two bleached
 * out. They now composite with plain alpha, so the scene reads as the same scene on
 * both grounds — lighter here, but present. This is the cell that proves it.
 */
 ...
 * `surface` under its two washes. Worth seeing on its own: it is what the
